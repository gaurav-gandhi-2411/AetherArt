"""Publish the SDXL Ukiyo-e LoRA adapter to Hugging Face Hub.

Usage:
    python scripts/publish_lora_sdxl.py \\
        --adapter-path data/lora/ukiyo-e/ukiyo-e-sdxl-lora.safetensors \\
        --repo-id gauravgandhi2411/aetherart-ukiyo-sdxl \\
        --dry-run
...
_MODEL_CARD_PATH = _REPO_ROOT / "docs" / "model_cards" / "sdxl_ukiyo_e.md"
_SAMPLE_IMAGES_DIR = _REPO_ROOT / "reports" / "lora_training_summary_sdxl_samples"
...
def _publish(adapter_path: Path, repo_id: str) -> None:
    ...
    api.upload_file(
        ...
        repo_type="model",
        token=token,
        commit_message="upload SDXL Ukiyo-e LoRA adapter weights (checkpoint-1000, rank-8)",
    )
    ...
    if _SAMPLE_IMAGES_DIR.exists():
        ...
        api.upload_folder(
            ...
            token=token,
            commit_message="upload checkpoint-1000 sample images (4 prompts × 3 checkpoints)",
        )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Publish SDXL Ukiyo-e LoRA to HF Hub")
    parser.add_argument(
        "--adapter-path",
        type=Path,
        default=_REPO_ROOT / "data" / "lora" / "ukiyo-e" / "ukiyo-e-sdxl-lora.safetensors",
        help="Path to the .safetensors adapter file",
    )
    parser.add_argument(
        "--repo-id",
        default="gauravgandhi2411/aetherart-ukiyo-sdxl",
    ...
    if args.dry_run:
        ...
        if _SAMPLE_IMAGES_DIR.exists():
            n = len(list(_SAMPLE_IMAGES_DIR.glob("*.png")))
            print(f"[DRY RUN]   4. upload_folder samples/ ({n} PNGs) -> {args.repo_id}/samples")
