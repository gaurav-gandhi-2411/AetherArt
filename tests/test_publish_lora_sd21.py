"""Tests for scripts/publish_lora_sd21.py — mocked, no Hub calls."""
...
def _import_module():
    """Import publish_lora_sd21 from scripts/."""
    import importlib.util
    import sys

    spec = importlib.util.spec_from_file_location(
        "publish_lora_sd21",
        Path(__file__).resolve().parent.parent / "scripts" / "publish_lora_sd21.py",
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["publish_lora_sd21"] = mod
...
class TestDryRun:
    ...
    def test_dry_run_prints_planned_actions(self, tmp_path, capsys):
        ...
        assert "testuser/test-repo" in captured.out
...
class TestPublish:
    def test_publish_calls_create_repo_with_exist_ok(self, tmp_path):
        ...
        with (
            ...
            _mod._publish(adapter, "testuser/test-repo")
    ...
    def test_publish_uploads_safetensors_and_readme(self, tmp_path):
        ...
        # First call: adapter weights
        first_kwargs = upload_calls[0].kwargs
        assert first_kwargs["path_in_repo"] == "test.safetensors"
        assert first_kwargs["repo_id"] == "testuser/test-repo"

        # Second call: model card as README.md
        second_kwargs = upload_calls[1].kwargs
        assert second_kwargs["path_in_repo"] == "README.md"
        assert second_kwargs["repo_id"] == "testuser/test-repo"

        # No upload_folder call since samples dir doesn't exist
