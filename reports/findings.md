...
| 6 | LoRA rank ablation (rank 4 / 8 / 16) | Spread <1 SE (0.3337–0.3394) — CLIP-blind to rank | LPIPS 0.45–0.50 between ranks; rank-4 > rank-8 in CLIP (underfitting paradox) | [findings](experiments/exp6_lora_rank/findings.md) |
| 7 | LoRA data size (20 / 40 / 80 images) | Spread <1 SE (0.3303–0.3383) — CLIP-blind to data volume | LPIPS vs 80-img: data-20 = 0.66, data-40 = 0.55; non-monotonic CLIP (data-20 > data-80) | [findings](experiments/exp7_lora_data_size/findings.md) |
| 8 | LoRA alpha / style scale (0.0–1.5) | Rises +4 SE from no-LoRA to active-LoRA; flat within 0.5–1.25 | No-LoRA LPIPS = 0.67 vs reference; adjacent steps uniform 0.38–0.45 across sweep | [findings](experiments/exp8_lora_alpha/findings.md) |
| 9 | LoRA trigger token (with / without "ukyowood") | Delta = −0.0008 (0.12 SE, pure noise) | LPIPS = 0.41 — trigger redirects LoRA firing in ways CLIP cannot see | [findings](experiments/exp9_lora_trigger/findings.md) |

**The refined picture:** CLIP is not fully blind to style — when the prompt explicitly names a style (Exp 8: "ukiyo-e woodblock print"), CLIP partially registers whether the image matches that description. But CLIP cannot resolve within the active-style range (alpha 0.5–1.25 all within noise while spanning 0.41 LPIPS units), and it is entirely blind to stylistic choices that have no prompt-side vocabulary (Exp 9: trigger token, Exp 4: stochastic vs deterministic sampler paths). Experiments 6 and 7 extend this to training-time decisions: LoRA rank and data volume both produce large perceptual changes (LPIPS 0.45–0.66) while CLIP stays flat across all conditions. A consistent cross-experiment pattern — underfitted models (rank-4, data-20) scoring as high or higher than fully-trained ones on CLIP — confirms that CLIP rewards semantic keyword matching, not visual quality.

**Practical implication for this project:** DPM-Solver++ at 30 steps, CFG=7, fp16, ControlNet strength=1.0, LoRA alpha=1.0 with trigger token are validated as the working defaults. CLIP can confirm semantic alignment at those defaults but cannot guide fine-tuning of any of the parameters that shape visual character. For LoRA training decisions (rank, data size), visual inspection or LPIPS against a reference image is required.

**Full cross-experiment writeup:** [`reports/clip_blindness.md`](clip_blindness.md) — evidence table, mechanistic explanation, the underfitting paradox, practical implications, and the summary chart.
